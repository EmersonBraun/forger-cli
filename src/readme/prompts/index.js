...
function showData() {
  console.clear();
  const filteredprerequisites = moduleToGenerate.prerequisites.map(r => r.name).join(', ');
  const filteredinstalling = moduleToGenerate.installing.map(m => m.step).join(', ');
  const filteredusage = moduleToGenerate.usage.map(m => m.step).join(', ');
